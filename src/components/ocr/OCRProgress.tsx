...
export function OCRProgress({ state, onCancel }: OCRProgressProps) {
  ...
  const getStageText = (status: string): string => {
    switch (status) {
      case 'optimizing': return t('optimizingImage', 'Optimizing Image...');
      case 'uploading': return t('uploadingImage', 'Uploading Image...');
      case 'processing': return t('processingImage', 'Processing Image...');
      case 'extracting': return t('extractingText', 'Extracting Text...');
      case 'classifying': return t('analyzingReceipt', 'Analyzing Receipt...');
  ...
  const getStageIcon = (status: string): any => {
    switch (status) {
      case 'optimizing': return 'image';
      case 'uploading': return 'upload-cloud';
      case 'processing': return 'cpu';
      case 'extracting': return 'type';
      case 'classifying': return 'search';
    ...
    <View style={[styles.container, { backgroundColor: surfaceColor }]}>
      <View style={styles.header}>
        <View style={styles.titleContainer}>
          <Feather 
            name={getStageIcon(state.state.status)} 
            size={20} 
            color={primaryColor} 
          />
