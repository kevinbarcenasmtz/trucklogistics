...
export const useAppTheme = (): AppThemeHelpers => {
  ...
  const helpers = useMemo(() => ({
    getBackground: (variant: 'screen' | 'card' | 'input' | 'modal' = 'screen') => {
      switch (variant) {
        case 'screen': return colors.screenBackground;
        case 'card': return colors.cardBackground;
        case 'input': return colors.inputBackground;
        case 'modal': return colors.modalBackground;
        default: return colors.screenBackground;
      }
    },
    ...
    getBorder: (variant: 'default' | 'focus' = 'default') => {
      switch (variant) {
        case 'default': return colors.borderDefault;
        case 'focus': return colors.borderFocus;
    ...
    getTruckSemantic: (type: 'fuel' | 'maintenance' | 'route' | 'cargo') => {
      switch (type) {
        case 'fuel': return colors.fuel;
        case 'maintenance': return colors.maintenance;
        case 'route': return colors.route;
        case 'cargo': return colors.cargo;
