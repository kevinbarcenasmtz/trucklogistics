...
export const useOnboardingTheme = (): OnboardingThemeHelpers => {
  ...
  // Base colors
  const backgroundColor = isDarkTheme ? themeStyles.colors.black_grey : themeStyles.colors.background;
  const textColor = isDarkTheme ? themeStyles.colors.white : themeStyles.colors.text.primary;
  const secondaryTextColor = isDarkTheme ? themeStyles.colors.grey : themeStyles.colors.text.secondary;
...
export const useAppTheme = () => {
  const { backgroundColor, textColor, secondaryTextColor, themeStyles, isDarkTheme } = useOnboardingTheme();
