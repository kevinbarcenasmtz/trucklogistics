...
export default function StatsScreen() {
  ...
  const renderError = () =>
    error && (
      <View style={styles.errorContainer}>
        <Text style={[styles.errorText, { color: themeStyles.colors.status.error }]}>{error}</Text>
  ...
  const renderStatCard = ({ title, value, change, icon, isPositive }: StatData) => (
    ...
    >
      <View
        style={[styles.statIconContainer, { backgroundColor: themeStyles.colors.greenThemeColor }]}
      >
  ...
  const renderPerformanceBar = ({ label, value, color }: PerformanceData) => (
    <Animated.View entering={FadeIn.duration(600).delay(200)} style={styles.performanceItem}>
